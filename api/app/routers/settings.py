...
async def get_sla_settings(db: DbSession, _admin: AdminUser) -> SlaSettingsOut:
    ...
    policies_rows = (await db.execute(select(SlaPolicy).order_by(SlaPolicy.id))).scalars().all()
    ...
    return SlaSettingsOut(
        ...
        policies=[
            SlaPolicyOut(priority=p.priority, resolution_hours=p.resolution_hours)
...
async def update_sla_policy(
    ...
    policy = (
        await db.execute(select(SlaPolicy).where(SlaPolicy.priority == priority.lower()))
    ).scalar_one_or_none()
...
async def create_holiday(
    ...
    existing = (
        await db.execute(select(SlaHoliday).where(SlaHoliday.date == body.date))
    ).scalar_one_or_none()
...
async def delete_holiday(
    ...
    holiday = (
        await db.execute(select(SlaHoliday).where(SlaHoliday.id == holiday_id))
    ).scalar_one_or_none()
