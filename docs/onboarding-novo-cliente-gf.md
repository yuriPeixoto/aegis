...
| 5s | 2026-07-01 | Código já existia (onboarding anterior incompleto — credenciais nunca ativadas no `.env`). Perfil Carvalima (tem `technical_due_at`, `AGUARDANDO_VALIDACAO_CLIENTE`). Auditoria encontrou 17 correções da Carvalima nunca portadas (segurança do webhook, SLA, dispatch timing, etc.) — todas corrigidas. Mesmo servidor que o Aegis, mas **sem** app Lumen em `/api` (diferente de carvalima) — `AliasMatch` não se aplica, só o fix do `Redirect permanent` incondicional no vhost `:80`. **Armadilhas de schema encontradas só no teste ponta-a-ponta em produção** (não apareciam em nenhuma auditoria de código, só rodando de verdade): coluna `support_tickets.modulo` nunca existiu no banco (embora o código sempre esperasse); `ticket_responses.is_pinned` ausente; `ticket_attachments.user_id` ainda `NOT NULL` (quebra anexo vindo do Aegis); roles Spatie `Equipe Unitop`/`Equipe Qualidade` nunca criadas (`RoleDoesNotExist` ao notificar). Todas corrigidas via SQL. Lição: auditar `\d` das tabelas reais e `roles`/`role_has_permissions` ANTES do teste ponta-a-ponta, não durante. |
| jsp | 2026-07-01 | Onboarding do zero, depois upgrade de perfil (2026-07-07). Perfil inicial frigonorte, migrado pra perfil Carvalima completo a pedido do cliente (motor de SLA + `AGUARDANDO_VALIDACAO_CLIENTE` + campo `modulo`) — ver [ADR-009](adr/009-technical-due-at-vs-estimated-completion-date.md), adendo. Mesmo servidor que o Aegis; tem app Lumen real em `/var/www/gestaofrotas-jsp/api`, mas **sem `Alias /api` configurado no vhost ainda** — sem conflito ativo hoje, mas se alguém adicionar o Alias no futuro, precisa ser `AliasMatch` com exclusão de `aegis/`. Slug da source: `gf-jsp`. Diferente do 5S, roles Spatie já existiam mas sem nenhuma permission de `tickets.*` vinculada — mesma classe de armadilha, causa diferente (role existe mas vazia, em vez de role inexistente). |
